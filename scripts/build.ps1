...
& .venv\Scripts\pyinstaller.exe --noconfirm --clean --distpath build --workpath build\work --specpath build --name jobpostings-server --onedir --paths backend --add-data "$frontendDist;frontend\dist" --add-data "$staticDir;app\static" --collect-data tzdata backend\run_server.py
