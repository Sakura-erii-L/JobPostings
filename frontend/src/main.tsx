...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {selected ? <CompanyDetailShell company={selected} onBack={backFromCompany} onState={updateState} onFollow={followCompany} editable={isAdmin} onUpdated={updateCompany} /> : !canViewPage ? <CompaniesPage companies={companies} jobs={jobs} query={query} setQuery={setQuery} onSearch={() => loadData()} onSync={isAdmin ? sync : undefined} syncing={syncing} onResearch={isAdmin ? researchCompanies : undefined} researching={researching} onOpen={openCompany} onExport={exportJobs} onImport={isAdmin ? () => setPage('import') : undefined} isAdmin={isAdmin} onChanged={loadData} /> : page === 'companies' ? <CompaniesPage companies={companies} jobs={jobs} query={query} setQuery={setQuery} onSearch={() => loadData()} onSync={isAdmin ? sync : undefined} syncing={syncing} onResearch={isAdmin ? researchCompanies : undefined} researching={researching} onOpen={openCompany} onExport={exportJobs} onImport={isAdmin ? () => setPage('import') : undefined} isAdmin={isAdmin} onChanged={loadData} /> : page === 'timeline' ? <TimelinePage events={timeline} onOpenCompany={openCompany} /> : page === 'applications' ? <ApplicationsPage applications={applications} onState={updateState} /> : page === 'import' ? <ImportPage onImported={async () => { flash('已加入处理队列'); await loadData(); setPage('queue') }} /> : page === 'admin' ? <AdminPage onNavigate={target => { setPage(target); setSelected(null) }} /> : page === 'queue' ? <QueuePage onSync={sync} syncing={syncing} /> : page === 'security' ? <AccountSecurityPage /> : page === 'review' ? <ReviewPage onResolved={async () => { await loadData() }} /> : <SettingsPage onSaved={flash} onSync={sync} syncing={syncing} />}
    </main>
  </div>
}
...
function CompanyViewBase({ company, onBack, onState, onFollow }: { company: CompanyDetail; onBack: () => void; onState: (id: string, state: string, favorite?: boolean) => void; onFollow: (id: string, followed?: boolean) => void }) {
  const facts = [
  ...
  return <><button className="back" onClick={onBack}>← 返回企业列表</button><PageHeader eyebrow="企业详情" title={company.display_name} description={`${TAG_LABELS[company.primary_industry] || company.primary_industry} · ${company.jobs.length} 个岗位`}><button className="secondary" onClick={() => onFollow(company.id)}>☆ 关注企业</button></PageHeader><div className="detail-layout"><section><div className="detail-card intro"><div className="large-avatar">{company.display_name.slice(0, 1)}</div><div><h2>企业概览</h2><p>{company.summary || '企业资料正在根据来源整理。'}</p><CompanyTags tags={company.tags} primaryIndustry={company.primary_industry} /><div className="chips alias-tags">{company.aliases.map(alias => <span key={alias}>{alias}</span>)}</div>{company.public_researched_at && <small className="research-time">公开信息检索于 {formatDate(company.public_researched_at)}</small>}</div></div>{findings.length > 0 ? <div className="detail-card public-findings"><div className="section-title"><h2>公开信息核查</h2><span>{findings.length} 条需留意</span></div><p className="research-disclaimer">以下内容来自公开报道或监管/司法公开来源，需结合原文核实，不等同于对企业作出司法结论。</p>{findings.map(finding => <article className={`public-finding ${finding.severity}`} key={finding.id}><div className="public-finding-head"><strong>{finding.title}</strong><span className={`severity ${finding.severity}`}>{finding.severity === 'high' ? '高关注' : finding.severity === 'medium' ? '中关注' : finding.severity === 'low' ? '低关注' : '待确认'}</span></div><p>{finding.summary}</p><div className="public-finding-meta">{finding.source_title && <span>{finding.source_title}</span>}{finding.published_at && <span>报道时间：{finding.published_at}</span>}<a href={finding.source_url} target="_blank" rel="noreferrer">查看原始来源 ↗</a></div>{finding.resolved_url && finding.resolved_url !== finding.source_url && <details className="resolved-source"><summary>查看跳转诊断地址</summary><code>{finding.resolved_url}</code></details>}</article>)}</div> : company.verification_status?.startsWith('public_web') && <div className="detail-card public-findings empty-public-findings"><div className="section-title"><h2>公开信息核查</h2><span>本次已检索</span></div><p>本次检索未发现带有可靠直接来源的重大负面公开信息；这不等于不存在相关信息，建议结合原始来源持续复核。</p></div>}{facts.length > 0 && <div className="detail-card"><div className="section-title"><h2>企业资料</h2><span>由模型根据证据整理</span></div><dl className="company-facts">{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></div>}{company.major_requirements?.length ? <div className="detail-card"><div className="section-title"><h2>企业需求专业</h2><span>{company.major_requirements.length} 项</span></div><div className="chips alias-tags">{company.major_requirements.map(major => <span key={major}>{major}</span>)}</div></div> : null}{sharedDetails.length ? <div className="detail-card"><div className="section-title"><h2>招聘公共信息</h2><span>{sharedDetails.length} 条来源</span></div>{sharedDetails.map(detail => <article className="shared-recruitment-detail" key={detail.id}><div className="shared-detail-head"><strong>{detail.batch_name || '未注明招聘批次'}</strong><span>{detail.recruitment_type && detail.recruitment_type !== 'unknown' ? detail.recruitment_type : '适用岗位未注明'}</span></div><dl className="job-details"><ValueBlock label="工作地点" value={detail.locations} /><ValueBlock label="薪资" value={formatSalary(detail.salary)} /></dl><div className="public-finding-meta">{detail.source_type && <span>{SOURCE_TYPE_LABELS[detail.source_type] || detail.source_type}</span>}{detail.observed_at && <span>信息时间：{formatDate(detail.observed_at)}</span>}{detail.source_url && <a href={detail.source_url} target="_blank" rel="noreferrer">查看原始来源 ↗</a>}</div></article>)}</div> : null}<div className="detail-card"><div className="section-title"><h2>招聘时间轴</h2><span>{datedEvents.length} 项</span></div>{datedEvents.length ? datedEvents.map(event => <TimelineEventCard event={event} key={event.id} />) : <div className="empty-inline">暂无明确时间事件</div>}</div>{uncertainEvents.length > 0 && <div className="detail-card"><div className="section-title"><h2>待确认时间的招聘活动</h2><span>{uncertainEvents.length} 项</span></div><p className="setting-help">这些活动保留在企业详情中，待补充可靠时间后再进入全局时间轴。</p>{uncertainEvents.map(event => <TimelineEventCard event={event} key={event.id} />)}</div>}<div className="detail-card"><div className="section-title"><h2>招聘岗位</h2><span>{company.jobs.length} 个岗位</span></div>{company.jobs.length ? company.jobs.map(job => <JobRow key={job.id} job={job} onState={onState} />) : <div className="empty-inline">该招聘信息未公布岗位名称</div>}</div></section><aside><div className="detail-card"><div className="section-title"><h2>来源与证据</h2><span>{company.evidences.length}</span></div>{company.evidences.map(evidence => <EvidenceCard key={evidence.id} evidence={evidence} />)}</div></aside></div></>
}
...
function CompanyView({ company, onBack, onState, onFollow }: { company: CompanyDetail; onBack: () => void; onState: (id: string, state: string, favorite?: boolean) => void; onFollow: (id: string, followed?: boolean) => void }) {
  ...
  return <><CompanyViewBase company={baseCompany} onBack={onBack} onState={onState} onFollow={onFollow} />{sharedDetails.length ? <section className="detail-card shared-recruitment-detail-full"><div className="section-title"><h2>招聘公共信息</h2><span>{sharedDetails.length} 条来源</span></div>{sharedDetails.map(detail => <article className="shared-recruitment-detail" key={detail.id}><div className="shared-detail-head"><strong>{detail.batch_name || '未注明招聘批次'}</strong><span>{[detail.batch_year, detail.batch_season].filter(Boolean).join(' · ') || (detail.recruitment_type && detail.recruitment_type !== 'unknown' ? detail.recruitment_type : '适用岗位未注明')}</span></div><SharedDetailFields detail={detail} /><div className="public-finding-meta">{detail.source_type && <span>{SOURCE_TYPE_LABELS[detail.source_type] || detail.source_type}</span>}{detail.observed_at && <span>信息时间：{formatDate(detail.observed_at)}</span>}{detail.source_url && <a href={detail.source_url} target="_blank" rel="noreferrer">查看原始来源 ↗</a>}</div></article>)}</section> : null}</>
}
