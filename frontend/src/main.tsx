...
function App() {
  ...
  const sync = async () => {
    if (syncing) return
    setSyncing(true)
    try { const result = await api<{ fetched: number; added: number; created: number; updated: number; duplicates: number }>('/admin/sync', { method: 'POST' }); flash(`同步完成：读取 ${result.fetched} 条，新增 ${result.created} 条，更新 ${result.updated} 条，重复 ${result.duplicates} 条`); await loadData() }
