...
function CompaniesPage({ companies, jobs, query, setQuery, onSearch, onSync, syncing, onResearch, researching, onOpen, onExport, onImport, isAdmin, onChanged }: { companies: Company[]; jobs: Job[]; query: string; setQuery: (value: string) => void; onSearch: () => void; onSync?: (force?: boolean) => void; syncing?: boolean; onResearch?: (force?: boolean) => void; researching?: boolean; onOpen: (id: string) => void; onExport: (format: 'xlsx' | 'csv' | 'json') => void; onImport?: () => void; isAdmin?: boolean; onChanged?: () => Promise<void> }) {
...
function QueuePage({ onSync, syncing }: { onSync: () => Promise<void>; syncing: boolean }) {
  ...
  const load = async () => {
    try {
      ...
      const backgroundTotal = Object.values(background).reduce((sum, values) => sum + Object.values(values).reduce((subtotal, count) => subtotal + Number(count || 0), 0), 0)
      setMessage(backgroundTotal ? `后台任务（不等同于新增企业）：企业整理待处理 ${Number(consolidate.pending || 0)} / 已完成 ${Number(consolidate.succeeded || 0)}，公开检索待处理 ${Number(research.pending || 0)} / 已完成 ${Number(research.succeeded || 0)}` : '')
    } catch (e) {
      setMessage((e as Error).message)
    }
  }
