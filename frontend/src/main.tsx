...
type RecruitmentSharedDetail = { id: string; batch_name?: string; batch_year?: number; batch_season?: string; recruitment_type?: string; locations: string[]; salary: Record<string, any>; target_graduation_years?: number[]; education_requirements?: string[]; major_requirements?: string[]; application_url?: string | null; deadline?: string | null; process?: string[]; benefits?: string[]; source_type?: string; source_url?: string; observed_at?: string }
...
type SourceReference = { raw_message_id?: string | null; connector_id?: string | null; source_group_id?: string | null; source_group_name?: string | null; external_message_id?: string | null; sender?: string | null; sent_at?: string | null; message_type?: string | null; original_text_available?: boolean; current_text_available?: boolean; artifact_count?: number; source_status?: string; tracememo_cache?: { available: boolean; id?: string; source_time?: string | null; updated_at?: string | null } | null }
type ProcessingQueueItem = { id: string; kind: string; raw_message_id?: string | null; company_id?: string | null; parent_job_id?: string | null; subtasks?: ProcessingQueueItem[]; task_payload?: ProcessingTaskPayload | null; status: string; stage: string; attempts: number; lease_until?: string | null; next_attempt_at?: string | null; processor?: string | null; error?: string | null; result_json?: string | null; result?: Record<string, any> | null; created_at: string; updated_at: string; connector_id?: string | null; source_group_id?: string | null; source_group_name?: string | null; message_type?: string | null; sender?: string | null; sent_at?: string | null; recognition_status?: string | null; recognized_at?: string | null; recognition_error?: string | null; text_preview?: string | null; original_text?: string | null; source_reference?: SourceReference | null }
type ProcessingStatusStats = Record<string, number>
type ProcessingQueue = { state: 'running' | 'paused'; stats: ProcessingStatusStats; stats_by_kind?: Record<string, ProcessingStatusStats>; source_recognition?: ProcessingStatusStats; background_tasks?: ProcessingStatusStats; background_by_kind?: Record<string, ProcessingStatusStats>; items: ProcessingQueueItem[]; total: number; job_total?: number }
...
function App() {
  ...
  useEffect(() => {
    ...
    const first = document.querySelector<HTMLButtonElement>('.sidebar.open .nav-button, .sidebar.open .logout')
    first?.focus()
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (!drawer) return
      const focusable = Array.from(drawer.querySelectorAll<HTMLElement>('button:not(:disabled), [href], input:not(:disabled), select:not(:disabled)'))
  ...
  useEffect(() => {
    const handleResize = () => { if (window.innerWidth > 680 && mobileNavOpen) { setMobileNavOpen(false); menuButtonRef.current?.focus() } }
  ...
  return <div className="app-shell">
    {mobileNavOpen && <button className="nav-backdrop" aria-label="关闭导航菜单" onClick={() => { setMobileNavOpen(false); window.setTimeout(() => menuButtonRef.current?.focus(), 0) }} />}
    <aside id="main-navigation" className={`sidebar${mobileNavOpen ? ' open' : ''}`}>
      <div className="logo"><span className="logo-mark">J</span><span>JobPostings</span></div>
      <button className="mobile-nav-close" aria-label="关闭导航菜单" onClick={() => { setMobileNavOpen(false); window.setTimeout(() => menuButtonRef.current?.focus(), 0) }}>关闭</button>
      <div className="side-caption">招聘信息工作台</div>
      <nav>
        <div className="nav-group"><span className="nav-group-label">求职</span><NavButton active={page === 'companies'} onClick={() => navigate('companies')} icon="⌂">企业与岗位</NavButton><NavButton active={page === 'timeline'} onClick={() => navigate('timeline')} icon="◷">招聘时间轴</NavButton><NavButton active={page === 'applications'} onClick={() => navigate('applications')} icon="✓">求职进度</NavButton></div>
        {isAdmin && <div className="nav-group"><span className="nav-group-label">数据管理</span><NavButton active={page === 'import'} onClick={() => navigate('import')} icon="＋">导入信息</NavButton><NavButton active={page === 'review'} onClick={() => navigate('review')} icon="!">待审核</NavButton><NavButton active={page === 'queue'} onClick={() => navigate('queue')} icon="≋">处理队列</NavButton></div>}
        <div className="nav-group nav-group-bottom"><NavButton active={page === 'settings'} onClick={() => navigate('settings', settingsSection)} icon="⚙">设置与管理</NavButton></div>
      </nav>
      <div className="sidebar-bottom"><div className="user-avatar">{user.email.slice(0, 1).toUpperCase()}</div><div><strong>{user.email}</strong><small>{user.role === 'admin' ? '管理员' : '受邀用户'}</small></div><button className="logout" aria-label="退出登录" onClick={async () => { await api('/auth/logout', { method: 'POST' }); location.reload() }}>↗</button></div>
    </aside>
...
function CompaniesPage({ companies, jobs, query, setQuery, onSearch, onResearch, researching, onOpen, onExport, onImport, isAdmin, onChanged, dataStatus }: { companies: Company[]; jobs: Job[]; query: string; setQuery: (value: string) => void; onSearch: () => void; onSync?: (force?: boolean) => void; syncing?: boolean; onResearch?: (force?: boolean) => void; researching?: boolean; onOpen: (id: string) => void; onExport: (format: 'xlsx' | 'csv' | 'json') => void; onImport?: () => void; isAdmin?: boolean; onChanged?: () => Promise<void>; dataStatus: Record<DataSection, DataSectionStatus> }) {
  ...
  const visibleCompanies = companies
    ...
    .sort((left, right) => {
      const updatedDesc = (right.updated_at || '').localeCompare(left.updated_at || '')
      const nameAsc = left.display_name.localeCompare(right.display_name, 'zh-Hans')
      if (sortBy === 'jobs_desc') return (right.job_count || 0) - (left.job_count || 0) || updatedDesc || nameAsc
      if (sortBy === 'name_asc') return nameAsc || updatedDesc
      if (sortBy === 'updated_asc') return -updatedDesc || nameAsc
      return updatedDesc || nameAsc
    })
  ...
  const manageCompanies = async (operation: 'merge' | 'delete') => {
    ...
    try {
      ...
      const confirmation = operation === 'merge'
...
function CompanyCard({ company, onClick, selectable = false, selected = false, selectionOrder = 0, motionEnabled = false }: { company: Company; onClick: () => void; selectable?: boolean; selected?: boolean; selectionOrder?: number; motionEnabled?: boolean }) {
  ...
  return <button type="button" className={`company-card${selected ? ' selected-company-card' : ''}${motionEnabled ? ' company-card-motion' : ''}`} onClick={onClick} onPointerEnter={jelly.onPointerEnter} onPointerMove={jelly.onPointerMove} onPointerLeave={jelly.onPointerLeave} onPointerCancel={jelly.onPointerCancel}><motion.div className="company-card-visual" style={jelly.style}><div className="company-top"><div className="company-avatar">{company.display_name.slice(0, 1)}</div>{selectable ? <span className="company-select-indicator" aria-label={selected ? `第 ${selectionOrder} 个选择` : '未选择'}>{selected ? selectionOrder : '○'}</span> : <span className="more">···</span>}</div><h3>{company.display_name}</h3><CompanyTags tags={company.tags} primaryIndustry={company.primary_industry} /><div className="chips company-card-meta"><span>{company.job_count} 个岗位</span></div><p>{company.summary || '企业介绍将在联网检索或审核后补充。'}</p><div className="card-footer"><span>{selectable ? (selected ? `选择顺序 ${selectionOrder}` : '点击选择') : '最近更新'}</span><time>{selectable ? '' : company.updated_at?.replace('T', ' ').slice(0, 16) || '—'}</time><span className="arrow">{selectable ? (selected ? '✓' : '＋') : '→'}</span></div></motion.div></button>
}
...
function TabbedCompanyView({ company, onBack, onState, onFollow }: { company: CompanyDetail; onBack: () => void; onState: (id: string, state: string, favorite?: boolean) => void; onFollow: (id: string, followed?: boolean) => void }) {
  const [tab, setTab] = useState<'jobs' | 'timeline' | 'profile'>('jobs')
  const [evidenceOpen, setEvidenceOpen] = useState(false)
  const findings = company.public_findings || []
  const sharedDetails = company.recruitment_shared_details || []
  const datedEvents = company.events.filter(event => Boolean(event.start_at))
  const uncertainEvents = company.events.filter(event => !event.start_at)
  const facts = [['企业全称', company.legal_name], ['企业性质', company.company_nature], ['成立时间', company.founded_at], ['企业规模', company.company_size], ['总部及办公地点', company.headquarters], ['主要业务', company.businesses?.join('、')], ['企业亮点', company.highlights?.join('、')], ['官方网站', company.website], ['官方招聘渠道', company.official_channels?.join('、')]].filter(([, value]) => value)
  return <><button className="back" onClick={onBack}>← 返回企业列表</button><PageHeader eyebrow="企业详情" title={company.display_name} description={`${TAG_LABELS[company.primary_industry] || company.primary_industry} · ${company.jobs.length} 个岗位`}><button className="secondary" onClick={() => onFollow(company.id)}>☆ 关注企业</button></PageHeader><div className="company-summary detail-card"><div className="large-avatar">{company.display_name.slice(0, 1)}</div><div><h2>{company.display_name}</h2><p>{company.summary || '企业资料正在根据来源整理。'}</p><CompanyTags tags={company.tags} primaryIndustry={company.primary_industry} /><span className="company-key-facts">{[company.headquarters, company.company_size, company.verification_status].filter(Boolean).join(' · ') || '关键事实未提供'}</span></div></div><div className="company-tabs" role="tablist" aria-label="企业详情视图"><button role="tab" aria-selected={tab === 'jobs'} className={tab === 'jobs' ? 'active' : ''} onClick={() => setTab('jobs')}>岗位</button><button role="tab" aria-selected={tab === 'timeline'} className={tab === 'timeline' ? 'active' : ''} onClick={() => setTab('timeline')}>招聘动态</button><button role="tab" aria-selected={tab === 'profile'} className={tab === 'profile' ? 'active' : ''} onClick={() => setTab('profile')}>企业资料</button></div><button className="secondary evidence-trigger" aria-expanded={evidenceOpen} onClick={() => setEvidenceOpen(true)}>查看来源与证据（{company.evidences.length}）</button><div className="detail-layout company-detail-layout"><section>{tab === 'jobs' && <><div className="detail-card"><div className="section-title"><h2>招聘公共条件</h2><span>{sharedDetails.length} 条来源</span></div>{sharedDetails.length ? sharedDetails.map(detail => <details className="shared-recruitment-detail" key={detail.id}><summary className="shared-detail-head"><strong>{detail.batch_name || '未注明招聘批次'}</strong><span>{[detail.batch_year, detail.batch_season].filter(Boolean).join(' · ') || '条件摘要'}</span></summary><SharedDetailFields detail={detail} /></details>) : <div className="empty-inline">暂无公共招聘条件</div>}</div><div className="detail-card"><div className="section-title"><h2>招聘岗位</h2><span>{company.jobs.length} 个岗位</span></div>{company.jobs.length ? company.jobs.map(job => <JobRow key={job.id} job={job} onState={onState} />) : <div className="empty-inline">该招聘信息未公布岗位名称</div>}</div></>}{tab === 'timeline' && <div className="detail-card"><div className="section-title"><h2>招聘动态</h2><span>{company.events.length} 项</span></div>{datedEvents.length ? datedEvents.map(event => <TimelineEventCard event={event} key={event.id} />) : <div className="empty-inline">暂无明确时间事件</div>}{uncertainEvents.length > 0 && <><h3 className="subsection-title">待确认时间的招聘活动</h3>{uncertainEvents.map(event => <TimelineEventCard event={event} key={event.id} />)}</>}</div>}{tab === 'profile' && <>{findings.length > 0 ? <div className="detail-card public-findings"><div className="section-title"><h2>公开信息核查</h2><span>{findings.length} 条需留意</span></div><p className="research-disclaimer">以下内容来自公开来源，需结合原文核实，不等同于对企业作出司法结论。</p>{findings.map(finding => <article className={`public-finding ${finding.severity}`} key={finding.id}><div className="public-finding-head"><strong>{finding.title}</strong><span className={`severity ${finding.severity}`}>{finding.severity === 'high' ? '高关注' : finding.severity === 'medium' ? '中关注' : '待确认'}</span></div><p>{finding.summary}</p><div className="public-finding-meta">{finding.source_title && <span>{finding.source_title}</span>}{finding.published_at && <span>报道时间：{finding.published_at}</span>}<a href={finding.source_url} target="_blank" rel="noreferrer">查看原始来源 ↗</a></div></article>)}</div> : <div className="detail-card empty-public-findings"><h2>公开信息核查</h2><p>暂无带有可靠直接来源的核查结果。</p></div>}<div className="detail-card"><div className="section-title"><h2>企业资料</h2><span>由模型根据证据整理</span></div>{facts.length ? <dl className="company-facts">{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : <div className="empty-inline">企业资料未提供</div>}</div>{company.major_requirements?.length ? <div className="detail-card"><div className="section-title"><h2>企业需求专业</h2><span>{company.major_requirements.length} 项</span></div><div className="chips alias-tags">{company.major_requirements.map(major => <span key={major}>{major}</span>)}</div></div> : null}</>}</section>{evidenceOpen && <aside><div className="detail-card evidence-panel"><button className="secondary" onClick={() => { setEvidenceOpen(false); window.setTimeout(() => document.querySelector<HTMLElement>(".evidence-trigger")?.focus(), 0) }}>关闭来源与证据</button><div className="section-title"><h2>来源与证据</h2><span>{company.evidences.length} 个来源</span></div>{company.evidences.length ? company.evidences.map(evidence => <EvidenceCard key={evidence.id} evidence={evidence} />) : <div className="empty-inline">暂无来源证据</div>}</div></aside>}</div></>
}
...
function CompanyView({ company, onBack, onState, onFollow }: { company: CompanyDetail; onBack: () => void; onState: (id: string, state: string, favorite?: boolean) => void; onFollow: (id: string, followed?: boolean) => void }) {
  const sharedDetails = company.recruitment_shared_details || []
  const baseCompany = { ...company, recruitment_shared_details: [] }
  return <TabbedCompanyView company={{ ...baseCompany, recruitment_shared_details: sharedDetails }} onBack={onBack} onState={onState} onFollow={onFollow} />
}

function formatSalary(value?: Record<string, any>) {
  if (!value || !Object.keys(value).length) return ''
  if (value.description) return String(value.description)
  const range = value.minimum && value.maximum ? `${value.minimum}–${value.maximum}` : value.minimum || value.maximum || ''
  return [value.currency, range, value.period].filter(Boolean).join(' ')
}

function ValueBlock({ label, value }: { label: string; value?: string | null | string[] | Record<string, any> }) {
...
function SharedDetailFields({ detail }: { detail: RecruitmentSharedDetail }) {
  return <dl className="job-details"><ValueBlock label="工作地点" value={detail.locations} /><ValueBlock label="薪资" value={formatSalary(detail.salary)} /><ValueBlock label="目标毕业年份" value={detail.target_graduation_years?.map(year => String(year))} /><ValueBlock label="学历要求" value={detail.education_requirements} /><ValueBlock label="专业要求" value={detail.major_requirements} /><ValueBlock label="投递链接" value={detail.application_url} /><ValueBlock label="截止日期" value={detail.deadline} /><ValueBlock label="招聘流程" value={detail.process} /><ValueBlock label="福利" value={detail.benefits} /></dl>
}
...
function TimelineEventCard({ event, onOpenCompany }: { event: RecruitmentEvent; onOpenCompany?: (id: string) => void }) {
  const statusLabel = event.status === 'ongoing' ? '正在进行' : event.status === 'upcoming' ? '即将开始' : event.status === 'historical' ? '已结束' : '时间待确认'
...
function TimelinePage({ events, onOpenCompany, status, onRetry }: { events: RecruitmentEvent[]; onOpenCompany: (id: string) => void; status: DataSectionStatus; onRetry: () => void }) {
  ...
  const visible = events.filter(event => Boolean(event.start_at) && (!filter || event.event_type === filter))
  ...
  const weekEvents = visible.filter(event => {
    ...
    const startKey = eventDateKey(event.start_at, event.timezone)
    if (!startKey) return false
    if (!event.end_at) return startKey >= weekStartKey && startKey <= weekEndKey
    const endKey = eventDateKey(event.end_at, event.timezone)
    return endKey ? startKey <= weekEndKey && endKey >= weekStartKey : startKey >= weekStartKey && startKey <= weekEndKey
  })
  const sorted = (items: RecruitmentEvent[]) => [...items]
  const eventIntersectsDay = (event: RecruitmentEvent, dayKey: string) => {
    const startKey = eventDateKey(event.start_at, event.timezone)
    if (!startKey) return false
    if (!event.end_at) return startKey === dayKey
    const endKey = eventDateKey(event.end_at, event.timezone)
    return endKey ? startKey <= dayKey && endKey >= dayKey : startKey === dayKey
  }
  const unavailable = !events.length && (status.loading || Boolean(status.error))
  return <><PageHeader eyebrow="招聘日程" title="招聘时间轴" description="集中查看宣讲、网申截止、笔试、面试和其他招聘节点。"><div className="timeline-view-switch"><button className={`filter${viewMode === 'list' ? ' active-filter' : ''}`} onClick={() => setViewMode('list')}>列表</button><button className={`filter${viewMode === 'week' ? ' active-filter' : ''}`} onClick={() => setViewMode('week')}>周视图</button></div></PageHeader><DataSectionNotice label="招聘时间轴" status={status} onRetry={onRetry} /><div className="toolbar"><select className="filter" value={filter} onChange={event => setFilter(event.target.value)}><option value="">全部事件</option>{types.map(value => <option value={value} key={value}>{value}</option>)}</select>{viewMode === 'week' && <div className="timeline-week-controls"><button className="secondary" onClick={() => setWeekStart(addDays(weekStart, -7))}>上一周</button><button className="secondary" onClick={() => setWeekStart(startOfWeek(new Date()))}>本周</button><button className="secondary" onClick={() => setWeekStart(addDays(weekStart, 7))}>下一周</button><span>{formatWeekRange(weekStart, weekDays[6])}</span></div>}</div>{viewMode === 'list' ? <div className="timeline-list">{unavailable ? null : visible.length ? sorted(visible).map(event => <TimelineEventCard event={event} onOpenCompany={onOpenCompany} key={event.id} />) : <div className="empty-state compact">暂无招聘时间事件</div>}</div> : <>{unavailable ? null : weekEvents.length ? <div className="timeline-week">{weekDays.map((day, index) => { const key = localDateKey(day); const dayEvents = sorted(weekEvents.filter(event => eventIntersectsDay(event, key))); return <section className={`timeline-day${key === todayKey ? ' today' : ''}`} key={key}><header><strong>{WEEKDAY_LABELS[index]}</strong>{key === todayKey && <span className="timeline-today">今天</span>}<time>{formatShortDate(day)}</time></header>{dayEvents.length ? dayEvents.map(event => <TimelineEventCard event={event} onOpenCompany={onOpenCompany} key={`${event.id}-${key}`} />) : <div className="timeline-day-empty">暂无活动</div>}</section>})}</div> : <div className="empty-state compact">本周暂无明确时间事件</div>}</>}</>
}

function ApplicationsPage({ applications, onState, status, onRetry }: { applications: Application[]; onState: (id: string, state: string, favorite?: boolean) => void; status: DataSectionStatus; onRetry: () => void }) { const columns = [['interested', '感兴趣'], ['applied', '已投递'], ['interview', '面试中'], ['offer', 'Offer']] as const; const [activeState, setActiveState] = useState<string>('interested'); const unavailable = !applications.length && (status.loading || Boolean(status.error)); return <><PageHeader eyebrow="我的行动" title="求职进度" description="把感兴趣的岗位，从看到变成投递和面试。" /><DataSectionNotice label="求职进度" status={status} onRetry={onRetry} /><div className="kanban-tabs" role="tablist" aria-label="求职进度状态">{columns.map(([state, title]) => <button key={state} role="tab" aria-selected={activeState === state} className={activeState === state ? 'active' : ''} onClick={() => setActiveState(state)}>{title}（{applications.filter(job => job.state === state).length}）</button>)}</div><div className="kanban" data-active-state={activeState}>{columns.map(([state, title]) => <ApplicationColumn key={state} title={title} state={state} jobs={applications.filter(job => job.state === state)} onState={onState} />)}</div>{!unavailable && !applications.length && <div className="empty-state compact"><div className="empty-icon">✓</div><h3>收藏岗位后，它们会出现在这里</h3><p>在企业详情中点击星标，即可开始记录求职进度。</p></div>}</> }
...
const sourceStatusLabel: Record<string, string> = {
  text_available: '正文已保存',
  attachment_available_no_text: '附件已保存，但未提取出正文',
  cache_available_no_text: '正文未保存，但缓存可回查',
  identifier_only: '仅保留来源标识',
}

function SourceReferenceCard({ reference }: { reference?: SourceReference | null }) {
  if (!reference) return null
  const cache = reference.tracememo_cache
  const status = sourceStatusLabel[reference.source_status || ''] || '来源状态未知'
  const note = reference.original_text_available
    ? '该来源的可读正文已保存。'
    : cache
      ? '该消息的可读正文未保存在本地，但 TraceMemo 缓存仍在；可在“导入信息 → 选择群聊记录导入”中按群聊、时间或消息编号回查。'
      : reference.artifact_count
        ? `该消息没有可读正文，但本地保留 ${reference.artifact_count} 个附件，可根据附件状态继续处理。`
        : '该消息没有可读正文；当前仅能依据下面的来源标识回查。'
  return <details className="queue-source-reference"><summary><strong>对应聊天记录</strong><span>{status}</span></summary><div className="queue-source-grid"><div><small>来源群聊</small><strong>{reference.source_group_name || reference.source_group_id || '未知'}</strong></div><div><small>消息时间</small><strong>{reference.sent_at ? formatDate(reference.sent_at) : '未知'}</strong></div><div><small>TraceMemo 消息编号</small><strong>{reference.external_message_id || '未知'}</strong></div><div><small>发送者</small><strong>{reference.sender || '未知'}</strong></div><div><small>本地原始消息 ID</small><code>{reference.raw_message_id || '未知'}</code></div>{cache && <div><small>本地缓存记录 ID</small><code>{cache.id || '未知'}</code></div>}</div><p className="queue-source-note">{note}</p></details>
}
...
function ImportPage({ onImported }: { onImported: () => Promise<void> }) {
  ...
  const [messageStep, setMessageStep] = useState<1 | 2 | 3>(1)
  ...
  return <><PageHeader eyebrow="数据入口" title="导入招聘信息" description="选择一种入口，系统会自动解析、分类、去重并保留来源。" /><div className="import-tabs" role="tablist" aria-label="导入方式"><button role="tab" aria-selected={mode === 'text'} className={mode === 'text' ? 'active' : ''} onClick={() => setMode('text')}>粘贴文字</button><button role="tab" aria-selected={mode === 'url'} className={mode === 'url' ? 'active' : ''} onClick={() => setMode('url')}>链接与文件</button><button role="tab" aria-selected={mode === 'messages'} className={mode === 'messages' ? 'active' : ''} onClick={() => setMode('messages')}>群聊记录</button></div><div className="import-single-panel">{mode === 'text' && <div className="detail-card import-card"><div className="import-icon blue">✎</div><h2>粘贴群聊文字</h2><p>适合复制微信群中的招聘消息、合并转发和招聘说明。</p><textarea value={text} onChange={e => setText(e.target.value)} rows={10} placeholder="粘贴招聘群消息……" /><button className="primary" disabled={busy || !text.trim()} onClick={submitText}>{busy ? '提交中…' : '加入处理队列 →'}</button></div>}{mode === 'url' && <div className="detail-card import-card"><div className="import-icon violet">↗</div><h2>公开链接与文件</h2><p>公众号文章、企业官网、招聘平台和其他公开页面均可。</p><input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://example.com/recruitment" /><div className="dropzone">将网页 URL 粘贴到上方<br /><span>验证码、登录页和小程序请手工补录</span></div><button className="secondary" disabled={busy || !url.trim()} onClick={submitUrl}>抓取并加入队列 →</button><label className="file-input">选择 PDF、DOCX、XLSX、CSV、TXT 或图片<input type="file" accept=".pdf,.docx,.xlsx,.csv,.txt,.png,.jpg,.jpeg,.webp" onChange={e => e.target.files?.[0] && submitFile(e.target.files[0])} /></label></div>}{mode === 'messages' && <section className="detail-card import-card import-messages-card"><div className="import-icon green">☷</div><h2>选择群聊记录导入</h2><p>步骤 1：选择群聊；步骤 2：读取并选择消息；步骤 3：确认入队。</p><TraceMemoGroups compact syncing={busy} onSaved={() => { void loadMessages(); setMessageStep(2) }} /><div className="import-step-controls" aria-label="群聊导入步骤"><span className={messageStep === 1 ? "active" : ""}>1 选择群聊</span><span className={messageStep === 2 ? "active" : ""}>2 选择消息</span><span className={messageStep === 3 ? "active" : ""}>3 确认入队</span>{messageStep > 1 && <button className="secondary" onClick={() => setMessageStep((messageStep - 1) as 1 | 2 | 3)}>上一步</button>}{messageStep < 3 && <button className="secondary" onClick={() => setMessageStep((messageStep + 1) as 1 | 2 | 3)}>下一步</button>}</div><div className="import-message-toolbar"><input className="message-search" value={messageQuery} onChange={e => setMessageQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && void loadMessages()} placeholder="搜索消息内容、发送人、群名或消息编号" /><button className="secondary" disabled={loadingMessages || busy} onClick={() => void loadMessages()}>{loadingMessages ? '读取中…' : '读取记录'}</button><label className="check"><input type="checkbox" checked={allSelected} onChange={() => setSelectedMessageIds(allSelected ? [] : selectableMessages.map(item => item.id))} />全选未导入</label></div>{messageNotice && <p className="setting-help admin-message">{messageNotice}</p>}{messages.length ? <div className="import-message-list">{messages.map(item => <label className={`import-message${item.imported ? ' imported' : ''}`} key={item.id}><input type="checkbox" disabled={busy || item.imported} checked={selectedMessageIds.includes(item.id)} onChange={event => toggleMessage(item.id, event.target.checked)} /><span className="import-message-copy"><strong>{item.group_name} · {item.sent_at ? formatDate(item.sent_at) : '时间未知'}</strong><span>{item.sender || '未知发送人'} · {item.message_type}{item.external_message_id ? ` · 消息编号 ${item.external_message_id}` : ''}</span><p>{item.text_preview || '（图片、文件或无文本消息）'}</p></span><span className={`status ${item.imported ? 'expired' : 'active'}`}>{item.imported ? '已导入' : '待导入'}</span></label>)}</div> : <div className="empty-inline">暂无可选记录。请先在上方勾选招聘群，并点击“读取记录”。</div>}<button className="primary" disabled={busy || !selectedMessageIds.length} onClick={() => { setMessageStep(3); void submitSelectedMessages() }}>{busy ? '导入中…' : `确认并导入 ${selectedMessageIds.length} 条记录 →`}</button></section>}</div></>
}

function CompactQueuePage({ onSync, syncing }: { onSync: () => Promise<void>; syncing: boolean }) {
  ...
  const load = async () => {
    try {
      ...
      const next = await api<ProcessingQueue>(path)
      setQueue(next)
      const background = next.background_by_kind || {}
      const consolidate = background.consolidate_company || {}
      const research = background.research_company || {}
      const followupTotal = [consolidate, research].reduce((sum, values) => sum + Object.values(values).reduce((subtotal, count) => subtotal + Number(count || 0), 0), 0)
      setMessage(followupTotal ? `后续步骤已归入来源任务：企业整理待处理 ${Number(consolidate.pending || 0)} / 已完成 ${Number(consolidate.succeeded || 0)}，公开检索待处理 ${Number(research.pending || 0)} / 已完成 ${Number(research.succeeded || 0)}` : '')
    } catch (e) {
      setMessage((e as Error).message)
    }
  }
  ...
  const stats = queue?.source_recognition || queue?.stats || {}
  ...
  const resultLabel = (item: ProcessingQueueItem) => {
    ...
    if (item.kind === 'classify' && result) {
      if (item.status === 'needs_review' && (!Number(result.company_count || 0) || result.error === '模型判断为招聘信息，但未产生可持久化企业数据')) return '需要处理：未提取到有效企业'
      if (result.is_recruitment === false) return '非招聘信息'
  ...
  const renderLogs = (id: string) => logs[id] ? <div className="processing-log queue-log">{logs[id].length ? logs[id].map(log => <div className={`log-line ${log.level}`} key={log.id}><time>{formatDate(log.created_at)}</time><strong>{log.stage}</strong><span>{log.message}</span>{Object.keys(log.details).length > 0 && <pre>{JSON.stringify(log.details, null, 2)}</pre>}</div>) : <div className="empty-inline">暂无阶段日志</div>}</div> : null
  const renderSubtask = (subtask: ProcessingQueueItem) => {
    const canRetry = ['needs_review', 'paused_quota', 'failed', 'canceled', 'timeout'].includes(subtask.status)
    const canCancel = ['pending', 'running', 'timeout', 'needs_review', 'paused_quota', 'failed'].includes(subtask.status)
    const actionLabel = subtask.status === 'timeout' ? '中断' : '取消'
    return <div className="queue-subtask" key={subtask.id}><div className="queue-subtask-head"><label className="queue-subtask-selector"><input type="checkbox" disabled={!canCancel} checked={selectedIds.includes(subtask.id)} onChange={() => setSelectedIds(current => current.includes(subtask.id) ? current.filter(id => id !== subtask.id) : [...current, subtask.id])} /><span className="sr-only">选择后续任务</span></label><div className="queue-subtask-copy"><strong>{kindLabel[subtask.kind] || subtask.kind}</strong><span>{subtask.status === 'running' ? '正在执行' : `阶段：${stageLabel[subtask.stage] || subtask.stage || '等待领取'}`}</span></div><div className="queue-item-actions"><span className={`status queue-status ${subtask.status}`}>{statusLabel[subtask.status] || subtask.status}</span>{canCancel && <button className="secondary danger" onClick={() => void cancel(subtask.id)}>{actionLabel}</button>}{canRetry && <button className="secondary" disabled={retrying === subtask.id} onClick={() => void retry(subtask.id)}>{retrying === subtask.id ? '重试中…' : '重试'}</button>}<button className="secondary" onClick={() => void toggleLogs(subtask.id)}>{logs[subtask.id] ? '收起日志' : '查看日志'}</button></div></div><div className={`queue-current-step ${subtask.status}`}><span>当前步骤</span><strong>{stageLabel[subtask.stage] || subtask.stage || '等待领取'}</strong></div>{resultLabel(subtask) && <p className="queue-result">处理结果：{resultLabel(subtask)}</p>}{subtask.error && <pre className="queue-error">{subtask.error}</pre>}{renderLogs(subtask.id)}</div>
  }
  const allQueueJobs = queue?.items.flatMap(item => [item, ...(item.subtasks || [])]) || []
  const cancellableIds = allQueueJobs.filter(item => ['pending', 'running', 'timeout', 'needs_review', 'paused_quota', 'failed'].includes(item.status)).map(item => item.id)
  const allSelected = cancellableIds.length > 0 && cancellableIds.every(id => selectedIds.includes(id))
  return <><PageHeader eyebrow="管理员" title="处理队列" description={`当前队列${queue?.state === 'running' ? '正在运行' : '已暂停'}；聊天时间来自原始消息，入队时间仅表示系统开始处理的时间。`}><button className="secondary" onClick={load}>↻ 刷新</button>{queue?.state === 'running' ? <button className="secondary" onClick={() => control('pause')}>暂停队列</button> : <button className="primary" onClick={() => control('run')}>继续处理</button>}<button className="secondary danger" onClick={() => control('cancel_all')}>取消全部未完成</button><button className="primary" disabled={syncing} onClick={() => onSync()}>{syncing ? '同步中…' : '从已选微信群获取'}</button></PageHeader><div className="metrics"><Metric label="等待处理" value={stats.pending || 0} tone="blue" /><Metric label="正在处理" value={stats.running || 0} tone="violet" /><Metric label="需要处理" value={attention} tone="orange" /><Metric label="已完成" value={stats.succeeded || 0} tone="green" /></div><div className="toolbar queue-toolbar"><select className="filter" value={filter} onChange={event => setFilter(event.target.value)}><option value="">全部未取消任务</option><option value="pending">等待处理</option><option value="running">处理中</option><option value="timeout">超时待人工操作</option><option value="needs_review">需要处理</option><option value="paused_quota">额度暂停</option><option value="canceled">已取消</option><option value="succeeded">已完成</option></select><label className="queue-select-all"><input type="checkbox" checked={allSelected} onChange={() => setSelectedIds(allSelected ? [] : cancellableIds)} />全选本页可取消任务</label><button className="secondary danger" disabled={!selectedIds.length} onClick={cancelSelected}>取消选中（{selectedIds.length}）</button><span className="queue-total">共 {queue?.total ?? '—'} 个主任务，底层 {queue?.job_total ?? '—'} 个步骤，页面每 3 秒自动刷新</span></div>{message && <div className="setting-help">{message}</div>}{queue?.items.length ? <div className="queue-list">{queue.items.map(item => { const canRetry = ['needs_review', 'paused_quota', 'failed', 'canceled', 'timeout'].includes(item.status); const canCancel = ['pending', 'running', 'timeout', 'needs_review', 'paused_quota', 'failed'].includes(item.status); const actionLabel = item.status === 'timeout' ? '中断' : '取消'; return <article className="detail-card queue-item" key={item.id}><div className="queue-item-head"><label className="queue-selector"><input type="checkbox" disabled={!canCancel} checked={selectedIds.includes(item.id)} onChange={() => setSelectedIds(current => current.includes(item.id) ? current.filter(id => id !== item.id) : [...current, item.id])} /><span className="sr-only">选择任务</span></label><div><strong>{kindLabel[item.kind] || item.kind}</strong><span>{item.source_group_name || (item.connector_id === 'manual' ? '手动导入' : '系统任务')} · 入队 {formatDate(item.created_at)}</span></div><div className="queue-item-actions"><span className={`status queue-status ${item.status}`}>{statusLabel[item.status] || item.status}</span>{canCancel && <button className="secondary danger" onClick={() => cancel(item.id)}>{actionLabel}</button>}{canRetry && <button className="secondary" disabled={retrying === item.id} onClick={() => retry(item.id)}>{retrying === item.id ? '重试中…' : '重试'}</button>}<button className="secondary" onClick={() => toggleLogs(item.id)}>{logs[item.id] ? '收起日志' : '查看日志'}</button></div></div><div className={`queue-current-step ${item.status}`}><span>当前步骤</span><strong>{stageLabel[item.stage] || item.stage || '等待领取'}</strong></div>{item.raw_message_id ? <><details className="queue-original"><summary><strong>聊天原文</strong><span>{item.original_text ? `${item.original_text.length} 字` : '未保存可读正文'} · 点击展开/收起</span></summary><pre>{item.original_text || '（未保存可读正文；请查看下方“对应聊天记录”）'}</pre></details><SourceReferenceCard reference={item.source_reference} /></> : <p className="queue-preview">{item.text_preview || (item.kind === 'consolidate_company' ? '等待合并企业的全部来源信息' : item.kind === 'research_company' ? '正在检索企业官网、行业属性和负面公开报道' : '无文本内容')}</p>}{item.subtasks?.length ? <div className="queue-subtasks"><div className="queue-subtasks-head">后续处理步骤</div>{item.subtasks.map(renderSubtask)}</div> : null}{resultLabel(item) && <p className="queue-result">处理结果：{resultLabel(item)}</p>}{item.error && <pre className="queue-error">{item.error}</pre>}<div className="queue-meta"><span>聊天时间：{item.sent_at ? formatDate(item.sent_at) : '未知'}</span><span>阶段：{item.stage || 'queued'}</span><span>识别：{recognitionLabel[item.recognition_status || ''] || item.recognition_status || '未关联原始消息'}</span><span>尝试 {item.attempts} 次</span>{item.processor && <span>{item.processor}</span>}<span>{item.message_type || item.kind}{item.sender ? ` · ${item.sender}` : ''}</span><span className="queue-id">{item.raw_message_id || item.company_id || item.id}</span></div>{logs[item.id] && <div className="processing-log queue-log">{logs[item.id].length ? logs[item.id].map(log => <div className={`log-line ${log.level}`} key={log.id}><time>{formatDate(log.created_at)}</time><strong>{log.stage}</strong><span>{log.message}</span>{Object.keys(log.details).length > 0 && <pre>{JSON.stringify(log.details, null, 2)}</pre>}</div>) : <div className="empty-inline">暂无阶段日志</div>}</div>}</article> })}</div> : <div className="empty-state compact"><div className="empty-icon">✓</div><h3>{filter ? '没有匹配的任务' : '处理队列为空'}</h3><p>手动导入测试信息或从已选微信群获取后，任务会在这里显示。</p></div>}</>
}

function AccountSecurityPage({ onDirtyChange, saveRequest, onSaveHandled }: { onDirtyChange?: (dirty: boolean) => void; saveRequest?: number; onSaveHandled?: (ok: boolean) => void } = {}) {
...
type SettingsEditorProps = { onSaved: (message: string) => void; onDirtyChange: (dirty: boolean) => void; saveRequest: number; onSaveHandled: (ok: boolean) => void }

function SettingsConnectionsPage({ onSaved, onDirtyChange, saveRequest, onSaveHandled, onSync, syncing }: SettingsEditorProps & { onSync: () => Promise<void>; syncing: boolean }) {
  ...
  useEffect(() => { Promise.all([api<Record<string, any>>('/admin/settings'), api<any[]>('/admin/connectors')]).then(([value, connectors]) => { setSettings(value); const connector = connectors.find(item => item.kind === 'tracememo'); if (connector) setTrace({ base_url: connector.base_url, enabled: Boolean(connector.enabled) }); setLoaded(true) }).catch(reason => { const message = (reason as Error).message; setError(message); setLoadError(message) }) }, [])
  const markServiceDirty = (service: string) => { setDirtyServices(current => new Set(current).add(service)); onDirtyChange(true) }
  const set = (key: string, value: any, service = 'agent') => { setSettings(current => ({ ...current, [key]: value })); markServiceDirty(service) }
  const setNested = (key: string, value: any, service: string) => set(key, value, service)
  const requestEdit = (next: string) => { if (next === editing) { setEditing(''); return } if (dirtyServices.size) { setPendingEdit(next); return } setEditing(next) }
  const save = async () => {
    setBusy('save'); setError('')
    try { const values: Record<string, any> = {}; if (dirtyServices.has('model')) values.llm_provider = settings.llm_provider; if (dirtyServices.has('backup')) values.backup = settings.backup; if (dirtyServices.has('smtp')) values.smtp = settings.smtp; if (dirtyServices.has('agent')) values.agent_api_enabled = settings.agent_api_enabled; if (Object.keys(values).length) await api('/admin/settings', { method: 'PUT', body: JSON.stringify({ values }) }); if (dirtyServices.has('tracememo')) await api('/admin/connectors/tracememo', { method: 'PUT', body: JSON.stringify({ ...trace, ...(traceToken ? { token: traceToken } : {}) }) }); setSettings(current => ({ ...current, llm_provider: { ...(current.llm_provider || {}), api_key: undefined }, backup: { ...(current.backup || {}), webdav_password: undefined, backup_password: undefined }, smtp: { ...(current.smtp || {}), password: undefined } })); setTraceToken(''); setDirtyServices(new Set()); onDirtyChange(false); setMessage('连接服务已保存'); onSaved('连接服务已保存'); if (pendingEdit) { setEditing(pendingEdit); setPendingEdit(null) } onSaveHandled(true) }
    catch (reason) { setError((reason as Error).message); setAdvancedOpen(true); window.setTimeout(() => document.querySelector<HTMLElement>('.service-editor input, .service-editor select')?.focus(), 0); onSaveHandled(false) }
    finally { setBusy('') }
  }
  useEffect(() => { if (saveRequest !== initialSaveRequest.current) { initialSaveRequest.current = saveRequest; if (loaded && dirtyServices.size) void save(); else onSaveHandled(false) } }, [saveRequest, loaded, dirtyServices.size])
  const test = async (kind: 'model' | 'backup') => { setBusy(kind); setError(''); setMessage(''); try { await api(kind === 'model' ? '/admin/models/test' : '/admin/backups/test', { method: 'POST' }); setMessage(kind === 'model' ? '通用模型连接成功' : 'WebDAV 连接成功') } catch (reason) { setError((reason as Error).message); setAdvancedOpen(true); window.setTimeout(() => document.querySelector<HTMLElement>('.service-editor input, .service-editor select')?.focus(), 0) } finally { setBusy('') } }
  if (loadError) return <div className="form-error" role="alert">加载连接服务失败：{loadError}</div>
  if (!loaded) return <div className="loading-inline">加载连接服务…</div>
  const provider = settings.llm_provider || {}; const backup = settings.backup || {}; const smtp = settings.smtp || {}
  const rows: Array<[string, string, string]> = [['tracememo', 'TraceMemo', trace.enabled ? '已启用' : '未启用'], ['model', '通用模型 API', provider.enabled ? '已启用' : '未启用'], ['backup', 'AList WebDAV', backup.enabled ? '已启用' : '未启用'], ['smtp', 'SMTP 邮件', smtp.enabled ? '已启用' : '未启用'], ['agent', 'Agent API', settings.agent_api_enabled ? '已启用' : '未启用']]
  return <><PageHeader eyebrow="连接服务" title="连接服务" description="先查看真实配置状态，再一次编辑一个服务。敏感值只在输入时存在。"><button className="primary" disabled={busy === 'save'} onClick={() => void save()}>{busy === 'save' ? '保存中…' : '保存连接设置'}</button></PageHeader>{(message || error) && <div className={error ? 'form-error' : 'setting-help'}>{error || message}</div>}{pendingEdit && <div className="unsaved-dialog" role="alertdialog"><strong>当前服务有未保存修改</strong><span>切换前请选择保存、放弃或留在当前服务。</span><div className="button-row"><button className="primary" disabled={busy === 'save'} onClick={() => void save()}>{busy === 'save' ? '保存中…' : '保存并切换'}</button><button className="secondary danger" onClick={() => { setDirtyServices(new Set()); onDirtyChange(false); setEditing(pendingEdit); setPendingEdit(null) }}>放弃修改</button><button className="secondary" onClick={() => setPendingEdit(null)}>留在当前页</button></div></div>}<div className="service-list">{rows.map(([id, label, status]) => <div className={`service-row${editing === id ? ' editing' : ''}`} key={id}><div><strong>{label}</strong><span>{status}{id === 'model' && provider.api_key_configured ? ' · API Key 已配置' : ''}{id === 'backup' && backup.webdav_password_configured ? ' · 密码已配置' : ''}</span></div><button className="secondary" onClick={() => requestEdit(id)}>{editing === id ? '收起' : '编辑'}</button></div>)}</div>{editing === 'tracememo' && <section className="detail-card setting-section service-editor"><h2>TraceMemo</h2><label className="check"><input type="checkbox" checked={Boolean(trace.enabled)} onChange={e => { setTrace({ ...trace, enabled: e.target.checked }); markServiceDirty('tracememo') }} />启用自动同步</label><label>API 地址<input value={trace.base_url} onChange={e => { setTrace({ ...trace, base_url: e.target.value }); markServiceDirty('tracememo') }} /></label><label>Bearer Token<input type="password" value={traceToken} placeholder="已配置时留空保持不变" onChange={e => { setTraceToken(e.target.value); markServiceDirty('tracememo') }} /></label><p className="setting-help">保存后可读取群聊列表并手工选择招聘群。Token 草稿不会写入 URL 或 localStorage。</p><TraceMemoGroups onSync={onSync} syncing={syncing} /></section>}{editing === 'model' && <section className="detail-card setting-section service-editor"><h2>通用模型 API</h2><label className="check"><input type="checkbox" checked={Boolean(provider.enabled)} onChange={e => setNested('llm_provider', { ...provider, enabled: e.target.checked }, 'model')} />启用云模型处理</label><label>API 风格<select value={provider.api_style || 'chat_completions'} onChange={e => setNested('llm_provider', { ...provider, api_style: e.target.value }, 'model')}><option value="chat_completions">Chat Completions</option><option value="responses">Responses</option></select></label><label>Base URL<input value={provider.base_url || ''} onChange={e => setNested('llm_provider', { ...provider, base_url: e.target.value }, 'model')} /></label><label>模型名称<input value={provider.model || provider.text_model || ''} onChange={e => setNested('llm_provider', { ...provider, model: e.target.value, text_model: e.target.value }, 'model')} /></label><details className="advanced-fields" open={advancedOpen}><summary>高级字段</summary><label>API Key<input type="password" placeholder={provider.api_key_configured ? '已配置，留空保持不变' : '输入 API Key'} onChange={e => setNested('llm_provider', { ...provider, api_key: e.target.value }, 'model')} /></label></details><button className="secondary" disabled={Boolean(busy)} onClick={() => void test('model')}>{busy === 'model' ? '测试中…' : '测试通用模型'}</button></section>}{editing === 'backup' && <section className="detail-card setting-section service-editor"><h2>AList WebDAV 备份</h2><label className="check"><input type="checkbox" checked={Boolean(backup.enabled)} onChange={e => setNested('backup', { ...backup, enabled: e.target.checked }, 'backup')} />启用每日备份</label><label>WebDAV URL<input value={backup.webdav_url || ''} onChange={e => setNested('backup', { ...backup, webdav_url: e.target.value }, 'backup')} /></label><label>用户名<input value={backup.username || ''} onChange={e => setNested('backup', { ...backup, username: e.target.value }, 'backup')} /></label><label>远端目录<input value={backup.remote_directory || '/JobPostings'} onChange={e => setNested('backup', { ...backup, remote_directory: e.target.value }, 'backup')} /></label><details className="advanced-fields" open={advancedOpen}><summary>高级字段</summary><label>WebDAV 密码<input type="password" placeholder={backup.webdav_password_configured ? '已配置，留空保持不变' : '输入 WebDAV 密码'} onChange={e => setNested('backup', { ...backup, webdav_password: e.target.value }, 'backup')} /></label><label>备份密码<input type="password" placeholder={backup.backup_password_configured ? '已配置，留空保持不变' : '输入独立备份密码'} onChange={e => setNested('backup', { ...backup, backup_password: e.target.value }, 'backup')} /></label></details><div className="button-row"><button className="secondary" disabled={Boolean(busy)} onClick={() => void test('backup')}>{busy === 'backup' ? '测试中…' : '测试 WebDAV'}</button><button className="secondary" disabled={Boolean(busy)} onClick={async () => { setBusy('run'); setError(''); try { await api('/admin/backups/run', { method: 'POST' }); setMessage('备份已完成') } catch (reason) { setError((reason as Error).message); setAdvancedOpen(true) } finally { setBusy('') } }}>立即备份</button></div></section>}{editing === 'smtp' && <section className="detail-card setting-section service-editor"><h2>SMTP 邮件</h2><label className="check"><input type="checkbox" checked={Boolean(smtp.enabled)} onChange={e => setNested('smtp', { ...smtp, enabled: e.target.checked }, 'smtp')} />启用邀请邮件</label><label>SMTP 主机<input value={smtp.host || ''} onChange={e => setNested('smtp', { ...smtp, host: e.target.value }, 'smtp')} /></label><label>端口<input type="number" value={smtp.port || 587} onChange={e => setNested('smtp', { ...smtp, port: Number(e.target.value) }, 'smtp')} /></label><label>发件人邮箱<input type="email" value={smtp.from_email || ''} onChange={e => setNested('smtp', { ...smtp, from_email: e.target.value }, 'smtp')} /></label><details className="advanced-fields" open={advancedOpen}><summary>高级字段</summary><label>用户名<input value={smtp.username || ''} onChange={e => setNested('smtp', { ...smtp, username: e.target.value }, 'smtp')} /></label><label>SMTP 密码<input type="password" placeholder={smtp.password_configured ? '已配置，留空保持不变' : '输入 SMTP 密码'} onChange={e => setNested('smtp', { ...smtp, password: e.target.value }, 'smtp')} /></label><label className="check"><input type="checkbox" checked={smtp.starttls !== false} onChange={e => setNested('smtp', { ...smtp, starttls: e.target.checked }, 'smtp')} />使用 STARTTLS</label></details></section>}{editing === 'agent' && <section className="detail-card setting-section service-editor"><h2>Agent API</h2><label className="check"><input type="checkbox" checked={Boolean(settings.agent_api_enabled)} onChange={e => set('agent_api_enabled', e.target.checked, 'agent')} />允许创建受限 Agent Token</label><p className="setting-help">Token 只能读取招聘目录和操作自己的求职进度，不能读取原始群消息或系统密钥。</p></section>}</>
}

function SettingsProcessingPage({ onSaved, onDirtyChange, saveRequest, onSaveHandled }: SettingsEditorProps) {
  const [settings, setSettings] = useState<Record<string, any>>({})
  const [loaded, setLoaded] = useState(false); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [loadError, setLoadError] = useState('')
  const initialSaveRequest = useRef(saveRequest)
  useEffect(() => { api<Record<string, any>>('/admin/settings').then(value => { setSettings(value); setLoaded(true) }).catch(reason => { const message = (reason as Error).message; setError(message); setLoadError(message) }) }, [])
  const set = (key: string, value: any) => { setSettings(current => ({ ...current, [key]: value })); onDirtyChange(true) }
  const save = async () => { setBusy(true); setError(''); try { const keys = ['sync_interval_minutes', 'import_days', 'redaction_enabled', 'processing_engine', 'local_ocr_fallback_enabled', 'model_concurrency', 'codex_concurrency', 'extract_concurrency', 'processing_log_retention_days']; const values = Object.fromEntries(keys.filter(key => key in settings).map(key => [key, settings[key]])); await api('/admin/settings', { method: 'PUT', body: JSON.stringify({ values }) }); onDirtyChange(false); setMessage('处理与隐私已保存'); onSaved('处理与隐私已保存'); onSaveHandled(true) } catch (reason) { setError((reason as Error).message); onSaveHandled(false) } finally { setBusy(false) } }
  useEffect(() => { if (saveRequest !== initialSaveRequest.current) { initialSaveRequest.current = saveRequest; if (loaded) void save(); else onSaveHandled(false) } }, [saveRequest, loaded])
  if (loadError) return <div className="form-error" role="alert">加载处理与隐私失败：{loadError}</div>
  if (!loaded) return <div className="loading-inline">加载处理与隐私…</div>
  return <><PageHeader eyebrow="处理与隐私" title="处理与隐私" description="控制同步范围、脱敏边界和本地处理并发。"><button className="primary" disabled={busy} onClick={() => void save()}>{busy ? '保存中…' : '保存处理设置'}</button></PageHeader>{(message || error) && <div className={error ? 'form-error' : 'setting-help'}>{error || message}</div>}<div className="settings-grid settings-single"><section className="detail-card setting-section"><h2>同步与隐私</h2><label>同步间隔（分钟）<input type="number" min="1" value={settings.sync_interval_minutes || 10} onChange={e => set('sync_interval_minutes', Number(e.target.value))} /></label><label>导入天数（距今）<input type="number" min="1" value={settings.import_days ?? settings.initial_import_days ?? 30} onChange={e => set('import_days', Math.max(1, Number(e.target.value) || 1))} /></label><p className="setting-help">每次同步导入距今设定天数内的聊天记录。</p><label className="check"><input type="checkbox" checked={Boolean(settings.redaction_enabled)} onChange={e => set('redaction_enabled', e.target.checked)} />发送云模型前启用脱敏（默认关闭）</label></section><section className="detail-card setting-section"><h2>处理器与并发</h2><label>招聘识别与整理处理器<select value={settings.processing_engine || 'codex'} onChange={e => set('processing_engine', e.target.value)}><option value="codex">本地 Codex（默认）</option><option value="generic">通用模型 API</option></select></label><p className="setting-help">切换只影响新任务和重新处理的任务。</p><label className="check"><input type="checkbox" checked={Boolean(settings.local_ocr_fallback_enabled)} onChange={e => set('local_ocr_fallback_enabled', e.target.checked)} />Codex OCR 失败时使用本地 OCR 兜底</label><label>模型并发（1–8）<input type="number" min="1" max="8" value={settings.model_concurrency || 2} onChange={e => set('model_concurrency', Math.max(1, Math.min(8, Number(e.target.value))))} /></label><label>Codex 并发（1–4）<input type="number" min="1" max="4" value={settings.codex_concurrency || 1} onChange={e => set('codex_concurrency', Math.max(1, Math.min(4, Number(e.target.value))))} /></label><label>本地提取并发<input type="number" min="1" max="16" value={settings.extract_concurrency || 4} onChange={e => set('extract_concurrency', Math.max(1, Math.min(16, Number(e.target.value))))} /></label><label>阶段日志保留天数<input type="number" min="1" value={settings.processing_log_retention_days || 30} onChange={e => set('processing_log_retention_days', Math.max(1, Number(e.target.value)))} /></label><button className="secondary" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { await api('/admin/models/test', { method: 'POST' }); setMessage('当前处理器连接成功') } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) } }}>测试当前处理器</button></section></div></>
}
