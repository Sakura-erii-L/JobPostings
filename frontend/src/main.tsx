...
type ProcessingQueueItem = { id: string; kind: string; raw_message_id?: string | null; status: string; attempts: number; lease_until?: string | null; error?: string | null; created_at: string; updated_at: string; connector_id?: string | null; source_group_id?: string | null; source_group_name?: string | null; message_type?: string | null; sender?: string | null; sent_at?: string | null; text_preview?: string | null }
type ProcessingQueue = { stats: Record<string, number>; items: ProcessingQueueItem[]; total: number }
...
function App() {
  ...
  const [page, setPage] = useState<'companies' | 'applications' | 'import' | 'admin' | 'queue' | 'settings' | 'review'>('companies')
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {selected ? <CompanyView company={selected} onBack={() => setSelected(null)} onState={updateState} onFollow={followCompany} /> : page === 'companies' ? <CompaniesPage companies={companies} jobs={jobs} query={query} setQuery={setQuery} onSearch={() => loadData()} onSync={user.role === 'admin' ? sync : undefined} syncing={syncing} onOpen={openCompany} onExport={exportJobs} onImport={() => setPage('import')} /> : page === 'applications' ? <ApplicationsPage applications={applications} onState={updateState} /> : page === 'import' ? <ImportPage onImported={async () => { flash('已加入处理队列'); await loadData(); setPage('queue') }} /> : page === 'admin' ? <AdminPage onNavigate={target => { setPage(target); setSelected(null) }} /> : page === 'queue' ? <QueuePage onSync={sync} syncing={syncing} /> : page === 'review' ? <ReviewPage onResolved={async () => { flash('审核项已处理'); await loadData() }} /> : <SettingsPage onSaved={flash} onSync={sync} syncing={syncing} />}
    </main>
  </div>
}
...
function CompaniesPage({ companies, jobs, query, setQuery, onSearch, onSync, syncing, onOpen, onExport, onImport }: { companies: Company[]; jobs: Job[]; query: string; setQuery: (value: string) => void; onSearch: () => void; onSync?: () => void; syncing?: boolean; onOpen: (id: string) => void; onExport: (format: 'xlsx' | 'csv' | 'json') => void; onImport: () => void }) {
...
function QueuePage({ onSync, syncing }: { onSync: () => Promise<void>; syncing: boolean }) {
  ...
  const statusLabel: Record<string, string> = { pending: '等待处理', running: '处理中', succeeded: '已完成', needs_review: '需要处理', paused_quota: '额度暂停', failed: '失败' }
  const kindLabel: Record<string, string> = { classify: '招聘识别', enrich_company: '企业补全' }

  return <><PageHeader eyebrow="管理员" title="处理队列" description="导入和微信群同步都会先进入这里；识别成功后才会出现在企业与岗位。"><button className="secondary" onClick={load}>↻ 刷新</button><button className="primary" disabled={syncing} onClick={onSync}>{syncing ? '同步中…' : '从已选微信群获取'}</button></PageHeader><div className="metrics"><Metric label="等待处理" value={stats.pending || 0} tone="blue" /><Metric label="正在处理" value={stats.running || 0} tone="violet" /><Metric label="需要处理" value={attention} tone="orange" /><Metric label="已完成" value={stats.succeeded || 0} tone="green" /></div><div className="toolbar queue-toolbar"><select className="filter" value={filter} onChange={event => setFilter(event.target.value)}><option value="">全部任务</option><option value="pending">等待处理</option><option value="running">处理中</option><option value="needs_review">需要处理</option><option value="paused_quota">额度暂停</option><option value="succeeded">已完成</option></select><span className="queue-total">共 {queue?.total ?? '—'} 个任务，页面每 3 秒自动刷新</span></div>{message && <div className="setting-help">{message}</div>}{queue?.items.length ? <div className="queue-list">{queue.items.map(item => { const canRetry = ['needs_review', 'paused_quota', 'failed'].includes(item.status); return <article className="detail-card queue-item" key={item.id}><div className="queue-item-head"><div><strong>{kindLabel[item.kind] || item.kind}</strong><span>{item.source_group_name || (item.connector_id === 'manual' ? '手动导入' : '未标识来源')} · {formatDate(item.created_at)}</span></div><div className="queue-item-actions"><span className={`status queue-status ${item.status}`}>{statusLabel[item.status] || item.status}</span>{canRetry && <button className="secondary" disabled={retrying === item.id} onClick={() => retry(item.id)}>{retrying === item.id ? '重试中…' : '重试'}</button>}</div></div><p className="queue-preview">{item.text_preview || '无文本内容'}</p>{item.error && <pre className="queue-error">{item.error}</pre>}<div className="queue-meta"><span>尝试 {item.attempts} 次</span><span>{item.message_type || 'unknown'}{item.sender ? ` · ${item.sender}` : ''}</span><span className="queue-id">{item.raw_message_id || item.id}</span></div></article> })}</div> : <div className="empty-state compact"><div className="empty-icon">✓</div><h3>{filter ? '没有匹配的任务' : '处理队列为空'}</h3><p>手动导入测试信息或从已选微信群获取后，任务会在这里显示。</p></div>}</>
}
...
function TraceMemoGroups({ onSync, syncing }: { onSync: () => Promise<void>; syncing: boolean }) {
