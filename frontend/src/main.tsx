...
function App() {
  ...
  const sync = async (force = false) => {
    ...
    try {
      ...
      flash(`同步完成：读取 ${result.fetched} 条，新增 ${result.created} 条，更新 ${result.updated} 条，重复 ${result.duplicates} 条，已识别跳过 ${result.recognized_skipped || 0} 条，过滤系统消息 ${result.filtered_system || 0} 条${cacheText}${mediaText}${resetText}`)
      await loadData()
    }
...
function TraceMemoGroups({ onSync, syncing, compact = false, onSaved }: { onSync?: (force?: boolean) => Promise<void>; syncing?: boolean; compact?: boolean; onSaved?: () => void }) {
  ...
  return <div className={`group-picker${compact ? ' compact' : ''}`}><div className="group-picker-head"><strong>招聘群选择</strong><span>{selectedCount}/20</span></div><div className="group-picker-actions"><button className="secondary" onClick={refresh}>读取群列表</button><button className="secondary" onClick={save} disabled={!groups.length}>保存选择</button>{onSync && <button className="primary" onClick={startSync} disabled={syncing || selectedCount === 0}>{syncing ? '获取中…' : '立即从已选微信群获取'}</button>}</div>{onSync && <><label className="check force-refetch"><input type="checkbox" checked={forceRefetch} onChange={event => setForceRefetch(event.target.checked)} />强制重新获取（清理旧数据和缓存后重新拉取）</label>{forceRefetch && <p className="setting-warning">执行前会自动创建本地备份；旧目录、队列、审核、证据、聊天记录和 TraceMemo 缓存会被清理，账号、设置和微信群选择会保留。</p>}</>}{groups.length > 0 && <div className="group-picker-toolbar"><input className="group-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索群名称或群标识" /><span>{visibleGroups.length} 个群聊</span></div>}<div className="group-options">{!groups.length ? <div className="group-empty">正在读取群聊；如果还没有群，请点击“读取群列表”</div> : !visibleGroups.length ? <div className="group-empty">没有匹配的群聊</div> : visibleGroups.map(group => { const source = avatarSource(group.avatar); return <label className={`group-option${group.selected ? ' selected' : ''}`} key={group.id}><input type="checkbox" checked={group.selected} onChange={event => toggle(group.id, event.target.checked)} /><span className="group-avatar-wrap">{source ? <img className="group-avatar-image" src={source} alt={`${group.name}头像`} loading="lazy" /> : <span className="group-avatar" aria-hidden="true">{initial(group.name)}</span>}</span><span className="group-option-copy"><strong>{group.name}</strong><small>{group.external_id}</small></span></label> })}</div><p className="setting-help">TraceMemo 当前群聊接口未提供头像时显示群名首字母；在此保存勾选后，下面的消息列表会按当前群聊重新读取。{message && ` ${message}`}</p></div>
}
