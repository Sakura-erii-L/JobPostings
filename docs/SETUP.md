...
使用 `scripts\start-dev.ps1` 启动后，浏览器访问 `http://127.0.0.1:17879`。如果数据库已经初始化，显示登录页是正常现象：输入管理员邮箱和密码即可进入工作台。已有旧账号如果尚未设置密码，需要在仍保持登录的会话中打开“账户安全”设置密码。首次初始化时没有创建过管理员的环境，才会显示“创建本机管理员”。
