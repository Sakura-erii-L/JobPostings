...
使用 `scripts\start-dev.ps1` 启动后，浏览器访问 `http://127.0.0.1:17879`。如果数据库已经初始化，显示登录页是正常现象：输入首次初始化时使用的管理员邮箱，申请并填写邮箱验证码即可进入工作台。首次初始化时没有创建过管理员的环境，才会显示“创建本机管理员”。

当前版本没有独立的 `/admin` 地址。管理员登录后，左侧导航会显示“管理台”“系统设置”和“待审核”：

- “管理台”用于邀请受邀用户或其他管理员，并查看邀请记录及有效期。
- “系统设置”用于配置 TraceMemo、模型、脱敏、SMTP 和 AList WebDAV。
- “待审核”用于处理低置信度识别、字段冲突和失败任务。

在“管理台”创建邀请后，受邀用户使用被邀请的邮箱申请登录验证码。邀请有效期为 72 小时；要实际收到邮件，必须先在“系统设置”配置 SMTP。个人使用时可以只保留本机管理员，不创建其他用户。
