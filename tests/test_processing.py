...
def test_message_processing_creates_catalog_and_enrichment_job(tmp_path, monkeypatch):
    ...
    assert db.one("SELECT kind FROM processing_jobs WHERE kind='enrich_company'")["kind"] == "enrich_company"
